...
public class EventInnerDriverSubmitMission extends ExecuteListener implements ExecuteListenerInterface {
		...
		try {
			...
			if (missionList != null && !missionList.isEmpty()) {
				for (int i = 0; i < missionList.size(); i++) {
					...
					String missionUpdateSql = "UPDATE BO_EU_SH_VEHICLEORDER_MISSION SET  MISSIONSTATUS = '3', QRLC= "
							+ useCarDistance + ",DAYPRICE= " + (int) dayFee + ",DAYOVERKILOMETERSPRICE= "
							+ totalOverKmFee + ",DAYOVERTIMEPRICE= " + totaloverTimeFee + ",USECARTIME=" + useCarTime
							+ ",TOTALMONEY=" + totalFee + ",OVERTIMENUM=" + totalOverTime + ",OVERERDISTANCENUM="
							+ totalOverKm + ",OTTIMEPRICE=" + (int) overTimePrice + ",OTLEAVERICE=" + (int) overKmPrice
							+ ",CCGYF=" + (int) outFee+" WHERE BINDID ='"
									+ bindId + "'";;
